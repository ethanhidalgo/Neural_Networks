...
# Pre-moved to device once at startup — avoids a redundant .to(dev) inside
# every forward pass and loss call (each .to() does a no-op copy-check).
JOINT_WEIGHTS = torch.tensor(
    [10.0 if n in HIP_ADJACENT else 1.0 for n in JOINT_NAMES],
    dtype=torch.float32).to(device)

# Bone offsets as (N,3) tensor for batched FK — also pre-moved to device
OFFSET_TENSOR = torch.tensor(
    np.stack([JOINT_OFFSET[n] for n in JOINT_NAMES]),
    dtype=torch.float32).to(device)
...
def batch_fk(R_batch):
    ...
    B   = R_batch.shape[0]
    dev = R_batch.device
    off = OFFSET_TENSOR                   # (N,3) — already on device
...
def ik_loss(pred_rep, gt_pos, f_fn, rep_dim):
    ...
    w       = JOINT_WEIGHTS.unsqueeze(0)                          # (1,N) — already on device
...
def train_one_rep(name, f_fn, rep_dim, train_loader,
    ...
    for it in range(1, total_iters + 1):
        ...
        opt.zero_grad(set_to_none=True)  # frees grad memory instead of zeroing
        loss.backward()
        opt.step()
