...
def batch_fk(R_batch):
    ...
    wpos = torch.zeros(B, N_JOINTS, 3, device=dev)
    wrot = torch.zeros(B, N_JOINTS, 3, 3, device=dev)

    # root
    wrot[:, 0] = R_batch[:, 0]
    # wpos[:,0] stays zero  (hip fixed)

    for i in range(1, N_JOINTS):
        pi = PARENT_IDX[i]
        wrot[:, i] = wrot[:, pi] @ R_batch[:, i]
        # parent_world_pos + parent_world_rot @ bone_offset_i
        wpos[:, i] = wpos[:, pi] + (wrot[:, pi] @ off[i].unsqueeze(-1)).squeeze(-1)
