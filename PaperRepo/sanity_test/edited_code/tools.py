...
def stereographic_unproject(a, axis=None):
    ...
    ans = torch.autograd.Variable(torch.zeros(batch, a.shape[1]+1)) #batch*6
...
def compute_rotation_matrix_from_ortho5d(a):
    ...
    proj_scale = torch.autograd.Variable(torch.FloatTensor(proj_scale_np)).view(1,3).repeat(batch,1) #batch,3
...
def get_sampled_rotation_matrices_by_quat(batch):
    #quat = torch.autograd.Variable(torch.rand(batch,4))
    quat = torch.autograd.Variable(torch.randn(batch, 4))
    matrix = compute_rotation_matrix_from_quaternion(quat)
    return matrix
    
def get_sampled_rotation_matrices_by_hpof(batch):
    
    theta = torch.autograd.Variable(torch.FloatTensor(np.random.uniform(0,1, batch)*np.pi)) #[0, pi]
    phi   = torch.autograd.Variable(torch.FloatTensor(np.random.uniform(0,2,batch)*np.pi))      #[0,2pi)
    tao   = torch.autograd.Variable(torch.FloatTensor(np.random.uniform(0,2,batch)*np.pi))      #[0,2pi)
...
def get_sampled_rotation_matrices_by_axisAngle( batch, return_quaternion=False):
    ...
    theta = torch.autograd.Variable(torch.FloatTensor(np.random.uniform(-1,1, batch)*np.pi)) #[0, pi] #[-180, 180]
    sin = torch.sin(theta)
    axis = torch.autograd.Variable(torch.randn(batch, 3))
