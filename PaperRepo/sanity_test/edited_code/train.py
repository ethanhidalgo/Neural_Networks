...
def test(model, sample_num, input_mode, histogram_bins_num, angle_group_num, label="model", out_weight_folder="./"):
    print("########TEST##########")
    with torch.no_grad():
        model.eval()
        ...
        str_angles = "(" + str(pose_errors[pose_error_order[sample_num - 20]].item()) + ", " + str(
        ...
        str_angles = "(" + str(geodesic_errors[geodesic_error_order[sample_num - 20]].item()) + " " + str(
            geodesic_errors[geodesic_error_order[sample_num - 1]].item()) + ")\n"
        for i in range(sample_num - 20, sample_num):
        ...
        for i in range(group_num):
            ...
            plt.hist([pose_error], bins=histogram_bins_num, log=True, alpha=0.5,
                     label=str(min_angle) + "_" + str(max_angle))
        plt.ylabel('Pose_errors')
        plt.legend(loc='upper right')
        plt.savefig(out_weight_folder + label + "_pose_errors.png", dpi=150, bbox_inches='tight')
        plt.clf()
        ...
        for i in range(group_num):
            ...
            plt.hist([geodesic_error], bins=histogram_bins_num, log=True, alpha=0.5,
                     label=str(min_angle) + "-" + str(max_angle))
        plt.ylabel('Geodesic_errors')
        plt.legend(loc='upper right')
        plt.savefig(out_weight_folder + label + "_geodesic_errors.png", dpi=150, bbox_inches='tight')
        plt.clf()

    model.train()


def train(model, input_mode="pose", loss_mode="pose", sampling_method="axis_angle", batch=64, total_iter=20000,
          out_weight_folder="../train/", label="model"):
    ...
    for iteration in range(total_iter):
        ...
        # if(iteration%5000 == 0):
    test(model, sample_num=2048, input_mode=input_mode, histogram_bins_num=10, angle_group_num=30, label=label,
         out_weight_folder=out_weight_folder)


out_weight_folder = "../train/test0000/"

# torch.cuda.set_device(0)  # removed: running on CPU / MPS
...
train(model_o6mp, input_mode="r_matrix", loss_mode="pose", sampling_method="quaternion", batch=64, total_iter=500001,
      out_weight_folder=out_weight_folder + "o6mp/", label="o6mp")
# print("################TEST ON ortho6d, input=r_matrix, loss=Geodesic#####################")
# model_o6mg = Model(is_linear=False, out_rotation_mode="ortho6d")
# train(model_o6mg, input_mode="r_matrix", loss_mode="geodesic", sampling_method="quaternion", batch=64,
#       total_iter=500001, out_weight_folder=out_weight_folder + "o6mg/", label="o6mg")

print("################TEST ON ortho5d, input=r_matrix, loss=pose#####################")
model_o5pp = Model(is_linear=False, out_rotation_mode="ortho5d")
train(model_o5pp, input_mode="r_matrix", loss_mode="pose", sampling_method="quaternion", batch=64, total_iter=500001,
      out_weight_folder=out_weight_folder + "o5mp/", label="o5mp")
# print("################TEST ON ortho5d, input=pose, loss=Geodesic#####################")
# model_o5pg = Model(is_linear=False, out_rotation_mode="ortho5d")
# train(model_o5pg, input_mode="r_matrix", loss_mode="geodesic", sampling_method="quaternion", batch=64,
#       total_iter=500001, out_weight_folder=out_weight_folder + "o5mg/", label="o5mg")

print("################TEST ON Quaternion, input=r_matrix, loss=pose#####################")
model_qmp = Model(is_linear=False, out_rotation_mode="Quaternion")
train(model_qmp, input_mode="r_matrix", loss_mode="pose", batch=64, sampling_method="quaternion", total_iter=500001,
      out_weight_folder=out_weight_folder + "qmp/", label="qmp")
# print("################TEST ON Quaternion, input=r_matrix, loss=geodesic#####################")
# model_qmg = Model(is_linear=False, out_rotation_mode="Quaternion")
# train(model_qmg, input_mode="r_matrix", loss_mode="geodesic", sampling_method="quaternion", batch=64, total_iter=500001,
#       out_weight_folder=out_weight_folder + "qmg/", label="qmg")

print("################TEST ON AxisAngle, input=r_matrix, loss=pose#####################")
model_amp = Model(is_linear=False, out_rotation_mode="AxisAngle")
train(model_amp, input_mode="r_matrix", loss_mode="pose", batch=64, sampling_method="quaternion", total_iter=500001,
      out_weight_folder=out_weight_folder + "amp/", label="amp")
# print("################TEST ON AxisAngle, input=r_matrix, loss=geodesic#####################")
# model_amg = Model(is_linear=False, out_rotation_mode="AxisAngle")
# train(model_amg, input_mode="r_matrix", loss_mode="geodesic", sampling_method="quaternion", batch=64, total_iter=500001,
#       out_weight_folder=out_weight_folder + "amg/", label="amg")

print("################TEST ON euler, input=r_matrix, loss=pose#####################")
model_emp = Model(is_linear=False, out_rotation_mode="euler")
train(model_emp, input_mode="r_matrix", loss_mode="pose", batch=64, sampling_method="quaternion", total_iter=500001,
      out_weight_folder=out_weight_folder + "emp/", label="emp")
# print("################TEST ON euler, input=r_matrix, loss=geodesic#####################")
# model_emg = Model(is_linear=False, out_rotation_mode="euler")
# train(model_emg, input_mode="r_matrix", loss_mode="geodesic", sampling_method="quaternion", batch=64, total_iter=500001,
#       out_weight_folder=out_weight_folder + "emg/", label="emg")
...
train(model_emp, input_mode="r_matrix", loss_mode="pose", batch=64, sampling_method="quaternion", total_iter=500001,
      out_weight_folder=out_weight_folder + "rmp/", label="rmp")
...
# train(model_emg, input_mode="r_matrix", loss_mode="geodesic", sampling_method="axis_angle", batch=64, total_iter=500001,
#       out_weight_folder=out_weight_folder + "escmg/", label="escmg")
...
# train(model_emg, input_mode="r_matrix", loss_mode="pose", sampling_method="quaternion", batch=64, total_iter=500001,
#       out_weight_folder=out_weight_folder + "qhmp/", label="qhmp")
# model_qhmg = Model(is_linear=False, out_rotation_mode="Quaternion_half")
# train(model_emg, input_mode="r_matrix", loss_mode="geodesic", sampling_method="quaternion", batch=64, total_iter=500001,
#       out_weight_folder=out_weight_folder + "qhmg/", label="qhmg")
