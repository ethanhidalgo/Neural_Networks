"""Train one representation and save results."""
import sys, torch, torch.nn as nn, numpy as np, math, pickle

REP_NAME = sys.argv[1]  # e.g. "6D"
...
def random_so3_batch(n):
    ...
    R = torch.zeros(n, 3, 3)
    R[:,0,0]=(t*x*x+c).squeeze(); R[:,0,1]=(t*x*y-s*z).squeeze(); R[:,0,2]=(t*x*z+s*y).squeeze()
    R[:,1,0]=(t*x*y+s*z).squeeze(); R[:,1,1]=(t*y*y+c).squeeze();  R[:,1,2]=(t*y*z-s*x).squeeze()
    R[:,2,0]=(t*x*z-s*y).squeeze(); R[:,2,1]=(t*y*z+s*x).squeeze(); R[:,2,2]=(t*z*z+c).squeeze()
    return R
...
def g_quat(M):
    ...
    def ci(i):
        v=M[:,i,0]+M[:,0,i]
        return torch.where(v>0,torch.ones(B,device=M.device),
               torch.where(v<0,-torch.ones(B,device=M.device), sgn32**(i+1)))
    q_z=torch.stack([
        ...
        torch.zeros(B,device=M.device)],dim=1)
    mask=(t.abs()>1e-7).unsqueeze(1)
    return torch.where(mask,q_nz,q_z)
...
for it in range(1, TOTAL+1):
    ...
    if it % LOG == 0:
        ...
        print(f"[{REP_NAME}] iter {it} err {err:.3f}", flush=True)
...
with torch.no_grad():
    for _ in range(100):
        M = random_so3_batch(1000).to(device)
        Mp = f_fn(net(M.reshape(1000,9)))
        all_errs.append(geodesic_deg(M,Mp).cpu().numpy())
final_errors = np.concatenate(all_errs)

pickle.dump({"mean_errors": mean_errors, "final_errors": final_errors},
            open(f"./result_{REP_NAME.replace('-','_')}.pkl","wb"))
print(f"[{REP_NAME}] DONE mean={final_errors.mean():.2f} max={final_errors.max():.2f}")
